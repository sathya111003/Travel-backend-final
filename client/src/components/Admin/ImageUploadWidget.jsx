...
const ImageUploadWidget = ({ label, value, onChange, placeholder = "https://images.unsplash.com/...", onRemove }) => {
  ...
  return (
    <div className="space-y-1.5 w-full">
      {label && <label className="text-[10px] font-bold text-white/30 uppercase tracking-widest block">{label}</label>}
      <div className="flex items-center gap-3 bg-white/[0.02] p-3 rounded-xl border border-white/[0.06] hover:border-white/[0.1] transition-all">
        ...
        <div className="flex-1 flex items-center gap-2">
          <input className="flex-1 bg-background border border-white/[0.06] px-3 py-2.5 rounded-lg text-xs text-white outline-none focus:border-primary/40 transition-colors placeholder:text-white/15" placeholder={placeholder} value={value} onChange={(e) => onChange(e.target.value)} />
