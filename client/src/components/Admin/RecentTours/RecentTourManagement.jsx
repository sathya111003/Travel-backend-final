...
import { Plus, Trash2, Clock, Type, FileText, Video, Music, Upload, X, Play } from 'lucide-react';
...
const MediaUploadWidget = ({ label, value, onChange, accept, icon: Icon, color, uploadFn, placeholder }) => {
  ...
  const handleFileChange = async (e) => {
    ...
    if (file.size > maxSize) { setError(`Max ${accept === 'video/*' ? '50MB' : '10MB'}`); return; }
    ...
    catch (err) { setError(err.response?.data?.message || 'Upload failed'); setFileName(''); }
    ...
    <div className="space-y-1.5">
      ...
      <div className="flex items-center gap-3 bg-white/[0.02] p-3 rounded-xl border border-white/[0.06]">
        {value ? (
          ...
          <>
            <span className="text-xs text-white/20 flex-1 truncate">{placeholder}</span>
...
const RecentTourManagement = () => {
  ...
  const [formData, setFormData] = useState({ title: '', image: '', days: '', description: '', videoUrl: '', audioUrl: '' });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try { await createRecentTour(formData); setFormData({ title: '', image: '', days: '', description: '', videoUrl: '', audioUrl: '' }); setShowAddForm(false); getTours(); }
    catch { alert('Failed to add tour'); }
  };
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/40 text-sm mt-1">Manage the homepage tour memories section.</p>
        </div>
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-card p-6 rounded-2xl border border-white/[0.06]">
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Duration</label>
              ...
              <ImageUploadWidget label="Tour Image" value={formData.image} onChange={(val) => setFormData({ ...formData, image: val })} placeholder="Upload or enter URL..." />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <MediaUploadWidget label="Tour Video" value={formData.videoUrl} onChange={(val) => setFormData({ ...formData, videoUrl: val })} accept="video/*" icon={Video} color="text-blue-400" uploadFn={uploadVideo} placeholder="MP4, WebM..." />
                <MediaUploadWidget label="Tour Audio" value={formData.audioUrl} onChange={(val) => setFormData({ ...formData, audioUrl: val })} accept="audio/*" icon={Music} color="text-accent" uploadFn={uploadAudio} placeholder="MP3, WAV..." />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="flex justify-end gap-3">
                <button type="button" onClick={() => setShowAddForm(false)} className="px-5 py-2.5 rounded-xl bg-white/[0.04] text-white/60 text-sm font-bold hover:bg-white/[0.06] transition-all">Cancel</button>
                <button type="submit" className="px-6 py-2.5 bg-primary text-white rounded-xl font-bold text-sm shadow-lg shadow-primary/20 hover:shadow-primary/30 transition-all">Save</button>
              </div>
            </form>
          </motion.div>
        )}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div key={t._id} className="bg-card p-5 rounded-2xl border border-white/[0.06] flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-start gap-2">
                  ...
                  <button onClick={() => handleDelete(t._id)} className="text-white/20 hover:text-red-400 transition-colors shrink-0"><Trash2 size={14} /></button>
                </div>
                <p className="text-[10px] font-bold text-primary uppercase mt-0.5">{t.days}</p>
                <p className="text-xs text-white/40 mt-1.5 line-clamp-2">{t.description}</p>
          ...
          {tours.length === 0 && <div className="col-span-full bg-card p-16 rounded-2xl text-center text-white/20 text-sm">No tour memories yet</div>}
        </div>
      )}
    </div>
  );
